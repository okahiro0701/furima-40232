class OrderAddress
  ...
  attr_accessor :item_id, :user_id, :zip_code, :prefecture_id, :city_name, :block_number, :building_name, :phone_number, :price
  attr_accessor :token

  with_options presence: true do
    validates :zip_code,  format: {with: /\A[0-9]{3}-[0-9]{4}\z/, message: "is invalid. Include hyphen(-)"}
    validates :phone_number, format: {with: /\A\d{11}\z/, message: "is invalid. disclude hyphen(-)"}
